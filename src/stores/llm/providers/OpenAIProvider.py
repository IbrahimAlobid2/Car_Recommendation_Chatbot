...
OpenAIProvider
==============
This class, `OpenAIProvider`, implements the `LLMInterface` and provides methods to interact with OpenAI's text and vision models.

Usage:
------
- Initializes with API credentials and optional Azure settings.
- Supports text generation with chat history for contextual responses.
- Processes images for vision-based text extraction.
- Provides helper methods for prompt construction, embedding, and request formatting.

Methods:
--------
1. **set_generation_model(model_id: str) -> None**
   - Sets the text generation model ID.

2. **set_vision_model(model_id: str) -> None**
   - Sets the vision model ID.

3. **set_embedding_model(model_id: str) -> None**
   - Sets the embedding model ID.

4. **process_text(text: str) -> str**
   - Pre-processes input text to ensure it adheres to character limits.

5. **process_image(uploaded_image) -> str**
   - Encodes an uploaded image into Base64 format.

6. **generate_text(prompt: str, chat_history: list = None, max_output_tokens: int = None, temperature: float = None, type_chat: str = "agent") -> str**
   - Generates a response based on the provided prompt and optional parameters.

7. **LLM_CHAT(max_output_tokens = None, temperature = None) -> AzureChatOpenAI | ChatOpenAI**
   - Initializes a chat-based interaction with OpenAI or Azure OpenAI.

8. **vision_to_text(uploaded_image) -> str**
   - Sends an image for vision-based analysis and extracts textual details.

9. **embed_text(text: str) -> list**
   - Embeds the given text using the configured embedding model.

10. **construct_prompt(prompt: str, role: str) -> dict**
   - Constructs a structured message for chat interactions.
