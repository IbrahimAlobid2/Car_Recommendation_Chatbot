...
GroqProvider
============
This class, `GroqProvider`, implements the `LLMInterface` and provides methods to interact with Groq for both text generation and vision tasks.

Usage:
------
- Initializes with API credentials and default parameters for text and vision processing.
- Supports text generation with context-aware chat history.
- Processes images for vision-based text extraction.
- Provides helper methods for prompt construction and request formatting.
...
3. **process_text(text: str) -> str**
   - Pre-processes input text to ensure it adheres to character limits.

4. **process_image(uploaded_image) -> str**
   - Encodes an uploaded image into Base64 format.

5. **generate_text(prompt: str, chat_history: list = None, max_output_tokens: int = None, temperature: float = None, type_chat: str = "RAG") -> str**
   - Generates a response based on the provided prompt and optional parameters.

6. **LLM_CHAT(max_output_tokens = None, temperature = None) -> ChatGroq**
   - Initializes a chat-based interaction with Groq.

7. **vision_to_text(uploaded_image) -> str**
   - Sends an image for vision-based analysis and extracts textual details.

8. **construct_prompt(prompt: str, role: str) -> dict**
...
- `LLMInterface` for enforcing method structures.
- `Groq` and `ChatGroq` for interacting with Groq APIs.
