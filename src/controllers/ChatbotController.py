...
ChatbotController
=================
This class, `ChatbotController`, manages user interactions with an AI-powered chatbot. It supports natural language processing (NLP) for conversation, SQL query handling, and image analysis through an integrated vision model.

Usage:
------
- Handles text-based conversations using a large language model (LLM).
- Processes images to extract relevant information.
- Executes SQL queries using an SQL agent.
- Maintains conversation history for contextual responses.

Methods:
--------
1. **get_conversation_history(session_id: str, user_id: str) -> str**
   - Retrieves stored conversation history.

2. **append_to_history(session_id: str, user_id: str, user_text: str, assistant_text: str) -> None**
   - Updates the conversation history with the latest user and assistant messages.

3. **process_uploaded_image(file: UploadFile) -> str**
   - Analyzes an uploaded image to extract details using a vision model.

4. **handle_sql_mode(user_prompt: str) -> str**
   - Processes SQL-related queries and returns results.

5. **react_agent(user_prompt: str, conversation_history: str = "", car_details: str = "") -> str**
   - Implements a ReAct (Reasoning + Action) agent for dynamic user query handling.
   - Uses Thought -> Action -> Observation -> Answer methodology.
   
Dependencies:
-------------
- `fastapi` for handling file uploads.
- `SQL_AgentController` for processing SQL queries.
- `LLMProviderFactory` for LLM-based interactions.
- `PromptTemplate` for generating structured prompts.
...
response = chatbot.react_agent("What is the latest car model available?")
print(response)
```
"""
...
class ChatbotController(DatabaseController):
    """
    ...
    def react_agent(
        ...
        if car_details:
            messages.append({
                "role": "assistant",
                "content": f"image uploaded details: {car_details}"
            })
